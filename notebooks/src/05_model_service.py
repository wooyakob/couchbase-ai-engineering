...
if CAPELLA:
    ...
    LLM_MODEL = os.getenv("CAPELLA_LLM_MODEL")
    EMBEDDING_MODEL = os.getenv("CAPELLA_EMBEDDING_MODEL")
else:
...
for chunk in stream:
    if not chunk.choices:  # some backends send a trailing usage-only chunk with no choices
        continue
