...
#
# **Running against Capella with AI Functions already configured?** `AI_FUNCTIONS_ENABLED`
# is read from whichever env file loads as `.env` (see `ENV_FILE` in the next cell) — it is
# NOT inferred from the cluster. `.env.capella.example` ships with this var commented out,
# so if you `cp .env.capella.example .env.capella`, uncomment `AI_FUNCTIONS_ENABLED=true`
# there too, then either `cp .env.capella .env` or run this notebook with
# `ENV_FILE=.env.capella`. Otherwise Path A below silently no-ops into portable mode
# regardless of what's enabled on the cluster.
#
# **Path A failing with a permissions or "Oops, something went wrong" error?** See
# [Troubleshooting → AI Functions](../docs/troubleshooting.md#ai-functions-notebook-04) —
# in short: your database credential needs a role with **all privileges** (Database Access
# → credential → Roles tab → Add New Role) assigned to it for AI Functions' internal
# `CURL()` call to be allowed; if that's already granted and every row instead comes back
# `{"errorType": "InternalServerError", ...}`, the issue has moved to the function's
# **associated model** (AI Functions tab → click the model) — check its key/token there.
...
if AI_FUNCTIONS_ENABLED:
    ...
    print("skipped: AI_FUNCTIONS_ENABLED is not 'true' in the env file this kernel loaded "
          "(check ENV_FILE / your .env) — set it there, even if AI Functions are enabled "
          "on the Capella cluster itself")
