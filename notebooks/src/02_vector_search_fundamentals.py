            ...
            "types": {
                "docs.chunks": {
                    ...
                    "properties": {
                        ...
                        # Stored so callers (e.g. apps/rag-api) can request
                        # "lineage.doc_id" back from a search hit to attribute a
                        # citation to its source document.
                        "lineage": {"enabled": True, "dynamic": False, "properties": {
                            "doc_id": {"enabled": True, "dynamic": False,
                                      "fields": [{"name": "doc_id", "type": "text",
                                                  "index": True, "store": True,
                                                  "analyzer": "keyword"}]},
                        }},
                    },
...
# SearchOptions.scope_name property internally (to fall back to the scope this
# request runs against) even though nothing here ever sets scope_name, so the
# warning fires on every scoped search regardless of notebook code. There's no
# real migration available on our side; silence this specific, known-noisy one.
...
# %% [markdown]
# Note the top hit: nothing in the corpus says "password", yet the credential-rotation chunk
