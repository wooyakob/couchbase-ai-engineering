...
Because everything speaks the OpenAI protocol, this is a configuration decision, not an architecture decision. Keep `LLM_BASE_URL`, `LLM_API_KEY`, `LLM_MODEL`, `EMBEDDING_MODEL`, `EMBEDDING_DIM` in config (see `.env.example`), and make sure `EMBEDDING_DIM` flows into your index definitions — mismatched dimensions are the #1 vector-search setup error.
