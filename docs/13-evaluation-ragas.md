...
Every chapter so far ended with a knob: chunk size (3), embedding model (4), k and hybrid boosts (5), prompt wording (6), memory recall depth (9), agent instructions (10–11). None of those knobs comes with a gauge.

LLM systems fail *quietly*: a chunking change that tanks recall produces no exception, just worse answers. Evaluation is the gauge, and it has to run continuously, because your corpus, your users, and your models all drift.
...
One operational gotcha: set `RAGAS_DO_NOT_TRACK=true` before the first `ragas` import. Ragas phones home around every judge call, its analytics endpoint no longer resolves, and the silently-swallowed ~30s DNS failure blocks the event loop per call — turning an eval that should take seconds into an apparent hang (see [Troubleshooting](troubleshooting.md)).

**The eval set** is the real asset. Start with 20–50 question/reference pairs written from real user questions (the `rag-api` app logs every production Q&A to `ai.evals.samples` precisely so you can promote real traffic into eval cases). Grow it every time you find a failure. It's data: keep it in Couchbase, and version cases like code.
...
![An eval run document stored on Couchbase Server, with pipeline config and metric scores](../images/server/servereval.png)
*Exactly the shape the code above writes: `pipeline` pins the config that produced these numbers, `metrics` is what you trend over time.*
...
## 13.7 Recap: Closing the Main Loop

The book's *main sequence* closes here. Data flows in (3–4), gets retrieved (5–6), enriched (7), generated over (8), remembered (9), governed (10), orchestrated (11), exposed (12) and measured (13), with the measurements stored where everything else lives, joined to the exact code and catalog versions that produced them.

That loop: **ship, observe, evaluate, fix, re-measure**, is AI engineering.

Two deep dives follow for when you need them: Chapter 14 if Ch. 5's Search-service vector index stops being enough at scale, Chapter 15 if an LLM step needs to guarantee schema-valid JSON rather than just usually producing it.
...
Next: [Chapter 14: Vector Index Architectures](14-vector-index-architectures.md).
