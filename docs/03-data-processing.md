...
1. **Capella's managed pipeline**. Capella AI Services' **Data Processing + Vectorization workflows** handle extract → chunk → embed → index for you (Ch. 4 covers configuration; this chapter covers the *decisions*, especially chunking, that you still own).
...
- **`metadata` fields are your retrieval filters** (Ch. 5's prefilters and Ch. 6's tenant scoping). If you'll ever need "only product = capella", capture it now.
...
Chunking sets the resolution of retrieval.

- Too big → chunks match everything vaguely and blow the context budget.
- Too small → answers get shredded across chunks.

The strategies (these are also exactly the options Capella's vectorization workflows offer: see [the docs](https://docs.couchbase.com/ai/build/vectorization-service/data-processing.html)):
...
A practical structure-aware chunker (markdown-heading first, size-capped second, often beating pure semantic chunking on technical docs):
...
Overlap (10–20%) hedges against answers straddling a boundary. Whatever you choose, **store the chunker's parameters in `pipeline_version`** and re-evaluate with Ragas when you change them (Ch. 13: `context_precision` and `context_recall` are chunking-quality meters).
...
Deletion is the half everyone forgets: when a source document is removed, its chunks must go too, or retrieval serves ghosts.

Lineage makes it a query:
...
Structuring is also an AI task now. Ch. 7's AI Functions run enrichment inside the database as a SQL++ `UPDATE`: classify, extract, summarize.
...
Batch pipelines drift; **Eventing** doesn't. The Eventing service runs JavaScript functions on document mutations: the database's own change-triggered compute.

The canonical AI use: mark chunks stale the moment their source changes, and let a small worker re-embed only what's flagged.
...
The same trigger pattern powers: TTL-on-inactivity for sessions, auto-summarization queues (write a summarize task when a session closes: Ch. 9), and change-data-capture into audit collections.

If you're on Capella with vectorization workflows, freshness is handled for you (Ch. 4); Eventing is how you get the same property for pipelines you own.

Notebook 13 deploys exactly this: a real Eventing function via the Python SDK's management API, on a cluster you connect to, watches it react to a live mutation, rather than showing the pseudocode above.

---

## 3.7 Unstructured Sources

For PDFs/HTML/office docs, extraction happens before everything above.

Capella's Data Processing service handles common formats natively within a workflow (S3 or Couchbase sources).

In your own pipelines, pair an extractor library (e.g. `unstructured`, `pypdf`) with §3.4. Extraction quality is corpus-specific, so spot-check extracted text *before* debating chunk sizes; garbage text with beautiful chunking is still garbage.
...
- Enrich with AI Functions in-database (Ch. 7); keep derived data fresh with Capella workflows or Eventing (Ch. 4).
