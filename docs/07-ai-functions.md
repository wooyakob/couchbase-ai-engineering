...
**Prerequisites** (see the [official docs](https://docs.couchbase.com/ai/build/ai-functions.html)):

- Couchbase Server 8.0+ cluster with the Query service (Capella, Developer Pro or Enterprise plan).
- A configured LLM integration: the Capella Model Service (Chapter 8), OpenAI, or AWS Bedrock.
- RBAC permissions to run queries on the target collections.
- Note: once an AI Function is deployed to an operational cluster, it cannot be removed — plan the rollout.
...
Treat `ai_completion` like inline prompt engineering: keep the prompts short, deterministic (`temperature` near 0 for transforms), and version them — if a prompt matters, it belongs in your Agent Catalog prompt library (Chapter 10) or at minimum in source control, not scattered through queries.
...
- **Validation**: LLM output is still LLM output. For classification, check the returned label is in your label set before trusting it downstream; keep a `enriched_by`/`enriched_at` audit field on materialized results.
- **Self-managed clusters**: AI Functions are a Capella AI Services feature. On self-managed clusters, implement the same pattern as a small SDK worker: `SELECT` batch → call your model → `UPDATE` back. The notebook for this chapter implements both paths with the same interface.

## 7.7 Where this fits

AI Functions are the *data processing* face of AI on Couchbase — they pair with:
