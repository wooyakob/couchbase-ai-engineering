...
An embedding model maps text (or images) to a fixed-length vector of floats such that semantically similar inputs land near each other. Similarity is computed with **cosine similarity**, **dot product**, or **euclidean (L2) distance** — your index must be built with the same metric family the model was trained for (Chapter 5).

The three facts that drive engineering decisions:

1. **Dimensions are fixed per model** — `text-embedding-3-small` → 1536, `intfloat/e5-mistral-7b-instruct` (Capella Model Service) → 4096. The vector index's `dims` must match exactly.
...
You call the embedding API and store the vector on the document (the `embed()` in Chapter 3's pipeline):

```python
from openai import OpenAI

client = OpenAI()  # or Capella Model Service, Chapter 8
...
### Managed: Capella Vectorization workflows
...
1. **Source** — a Couchbase collection (structured JSON) or an S3 bucket (unstructured files: PDFs and friends, which the Data Processing service converts to JSON first).
...
From then on the workflow watches the source: new and changed documents get (re)processed and (re)embedded automatically. The staleness problem — the hard 20% of DIY — is the managed feature.

**When to use which:**

| | DIY pipeline | Vectorization workflow |
|---|---|---|
| Custom chunking/extraction logic | ✅ anything you can code | limited to offered strategies |
| Freshness on change | you build it (Eventing/queues) | ✅ automatic |
| Self-managed clusters | ✅ | ❌ (Capella AI Services) |
| Index creation | you write the JSON (Ch. 5) | ✅ generated |
| Odd sources (proprietary formats) | ✅ | S3 + common formats |

A common hybrid: workflows for the big steady corpora (docs, tickets), DIY for the exotic sources — both landing in the same collections, queried identically in Chapter 5. **Once vectors exist, nothing downstream cares who made them.**
...
Decision inputs, in the order they usually decide it:
