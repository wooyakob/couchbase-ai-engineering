...
async def config_error_handler(request: Request, exc: RuntimeError):
    # Every RuntimeError raised in this app (db.py, config.py, retrieval.py) is a
    # deliberately clear, actionable message about broken config/connectivity —
    # surface it in the response instead of a generic 500, so `curl` shows the fix
    # directly instead of sending you to the server logs.
