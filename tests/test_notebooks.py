...
def _has_eventing() -> bool:
    """The Eventing service is optional on both Server and Capella (notebook 13)."""
    return os.getenv("CB_SUPPORTS_EVENTING", "").lower() == "true"


def _has_analytics() -> bool:
    """The Analytics service is optional on both Server and Capella (notebook 14)."""
    return os.getenv("CB_SUPPORTS_ANALYTICS", "").lower() == "true"
...
def _has_uv() -> bool:
    """notebook 12 spawns the Couchbase MCP server via `uvx`, not a pip package."""
    import shutil
...
NEEDS_EVENTING = pytest.mark.skipif(not _has_eventing(), reason="CB_SUPPORTS_EVENTING is not "
                                   "'true' (cluster needs the Eventing service enabled)")
NEEDS_ANALYTICS = pytest.mark.skipif(not _has_analytics(), reason="CB_SUPPORTS_ANALYTICS is "
                                     "not 'true' (cluster needs the Analytics service enabled)")
...
NEEDS_UV = pytest.mark.skipif(not _has_uv(), reason="uv/uvx not installed "
                             "(https://docs.astral.sh/uv/), needed to run the MCP server")
...
@NEEDS_LLM
@NEEDS_UV
def test_12_mcp_server():
    run_notebook("12_mcp_server.ipynb")


@NEEDS_CB
@NEEDS_EVENTING
def test_13_eventing():
    run_notebook("13_eventing.ipynb")


@NEEDS_CB
@NEEDS_LLM
@NEEDS_ANALYTICS
def test_14_analytics_agent_tool():
    run_notebook("14_analytics_agent_tool.ipynb")
